-- Migration: Add before_quantity and after_quantity tracking to stock movements
-- Date: 2025-01-29
-- Description: Update consume_materials_on_semi_production to track before/after quantities

-- Drop and recreate the function with before/after quantity tracking
...
BEGIN
  ...
  -- Get before quantity for produced semi product
  SELECT quantity INTO v_before_qty
  FROM semi_finished_products
  ...
  -- Yarı mamul üretim stok hareketi kaydet with before/after tracking
  INSERT INTO stock_movements (
    material_type,
    material_id,
    movement_type,
...
-- Add comment
COMMENT ON FUNCTION consume_materials_on_semi_production() IS 'Updated to track before_quantity and after_quantity for all stock movements';
