-- 🔧 RECREATE MAINTENANCE FUNCTIONS V2: Bakım fonksiyonlarını yeniden oluştur
-- Bu dosya sistem bakım fonksiyonlarını önce kaldırıp sonra yeniden oluşturur

-- 1. Mevcut fonksiyonları kaldır
DROP FUNCTION IF EXISTS clean_old_audit_logs(INTEGER);
DROP FUNCTION IF EXISTS clean_old_notifications(INTEGER);
DROP FUNCTION IF EXISTS get_system_metrics();
DROP FUNCTION IF EXISTS check_system_health();
DROP FUNCTION IF EXISTS update_database_stats();
DROP FUNCTION IF EXISTS full_system_cleanup(INTEGER, INTEGER);

-- 2. Audit log temizleme fonksiyonu
...
-- 3. Bildirim temizleme fonksiyonu
...
-- 4. Sistem metrikleri fonksiyonu
...
-- 5. Sistem sağlık kontrolü fonksiyonu
...
-- 6. Veritabanı istatistikleri güncelleme fonksiyonu
...
-- 7. Tam sistem temizliği fonksiyonu
...
SELECT '✅ MAINTENANCE FUNCTIONS RECREATED V2!' as result;
