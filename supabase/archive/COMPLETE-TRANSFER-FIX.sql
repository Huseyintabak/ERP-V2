-- 🔧 COMPLETE TRANSFER FIX: Remove ALL Audit Triggers and Recreate Transfer Function
-- Bu dosya tüm audit trigger'larını kaldırır ve transfer fonksiyonunu yeniden oluşturur

-- 1. Tüm audit trigger'ları kaldır
...
-- 2. Audit fonksiyonlarını kaldır
...
-- 3. Transfer fonksiyonunu tamamen kaldır
DROP FUNCTION IF EXISTS transfer_between_zones(UUID, UUID, UUID, INTEGER, UUID);

-- 4. Yeni transfer fonksiyonunu oluştur (audit log olmadan)
...
-- 5. Tüm audit log tablolarını kontrol et ve gerekirse kaldır
DROP TABLE IF EXISTS audit_logs CASCADE;

SELECT '✅ COMPLETE TRANSFER FIX APPLIED - ALL AUDIT LOGS REMOVED!' as result;
