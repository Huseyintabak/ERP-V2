-- Fix transfer_between_zones function to match existing zone_transfers schema
...
DECLARE
  current_qty NUMERIC;
BEGIN
  -- Check if source zone has enough quantity (use zone_inventories)
  ...
  -- Record transfer (use existing schema: product_id, transfer_date)
  INSERT INTO zone_transfers (
    from_zone_id, 
    to_zone_id, 
    product_id,  -- Use product_id (not material_id)
    quantity, 
    transfer_date,  -- Use transfer_date (not created_at)
    created_by
  )
  VALUES (
    from_zone, 
...
SELECT '✅ TRANSFER_BETWEEN_ZONES V2 FIXED!' as result;
